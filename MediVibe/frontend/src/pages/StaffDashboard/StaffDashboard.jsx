...
import { FaHospital, FaSignOutAlt, FaBed, FaUsers, FaClipboard, FaHome, FaSync } from 'react-icons/fa';
...
const StaffDashboard = () => {
  ...
  const [admissionForm, setAdmissionForm] = useState({
    firstName: '', lastName: '', email: '', phone: '', age: '', dateOfBirth: '', gender: 'Male', bloodType: 'O+', address: '', ward: 'General', admissionReason: '', emergencyContact: { name: '', phone: '', relation: '' }
  });
  ...
  const staffStats = [
    { title: 'Available Beds', value: bedStats.reduce((sum, w) => sum + (w.available || 0), 0), icon: FaBed, color: 'bg-green-500' },
    { title: 'Occupied Beds', value: bedStats.reduce((sum, w) => sum + (w.occupied || 0), 0), icon: FaUsers, color: 'bg-teal-600' },
    { title: 'Patients (Active)', value: patients.length, icon: FaClipboard, color: 'bg-yellow-500' },
  ];
  ...
  const handleRefresh = async () => {
    ...
    setError('');
    try {
      if (activeTab === 'beds') await fetchBedStats();
      else if (activeTab === 'patients') await fetchPatients();
      else if (activeTab === 'overview') await Promise.all([fetchBedStats(), fetchPatients()]);
      else await Promise.all([fetchBedStats(), fetchPatients()]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const admitPatient = async () => {
    setLoading(true);
    setError('');
    if (!admissionForm.firstName || !admissionForm.lastName || !admissionForm.email || !admissionForm.phone || !admissionForm.dateOfBirth || !admissionForm.age) {
      setError('Please fill required fields: First/Last Name, Email, Phone, Age, Date of Birth');
      setLoading(false);
      return;
    }
    try {
      ...
      if (res?.data?.success) {
        setAdmissionForm({ firstName: '', lastName: '', email: '', phone: '', age: '', dateOfBirth: '', gender: 'Male', bloodType: 'O+', address: '', ward: 'General', admissionReason: '', emergencyContact: { name: '', phone: '', relation: '' } });
        fetchPatients();
        fetchBedStats();
      ...
      setError(`${apiError} - Ensure selected ward has beds and data is valid.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (activeTab === 'beds' || activeTab === 'overview') fetchBedStats();
      ...
      <div className="fixed left-0 top-0 w-64 h-screen bg-gradient-to-b from-slate-900 via-slate-800 border-r border-teal-500 border-opacity-20 p-6 overflow-y-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-teal-600">Staff Portal</p>
          </div>
        </div>
        ...
        <div className="border-t border-slate-700 pt-6 mt-6">
          <div className="bg-slate-800 bg-opacity-50 rounded-lg p-4 mb-4">
            ...
            {user?.specialization && <p className="text-xs text-stone-400">{user.specialization}</p>}
          </div>
          <button
      ...
      <div className="ml-64 p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-stone-400">Staff Dashboard - {user?.role.charAt(0).toUpperCase() + user?.role.slice(1)}</p>
          </div>
        ...
        {activeTab === 'overview' && (
          <div>
            ...
            {/* Quick Actions */}
            <div className="backdrop-blur-xl bg-stone-800 bg-opacity-40 border border-teal-500 border-opacity-30 rounded-xl p-6 mb-6">
              <h3 className="text-xl font-bold text-white mb-4">Quick Actions</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button className="bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white px-6 py-3 rounded-lg transition font-semibold">
                  Admit Patient
                </button>
                <button className="bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white px-6 py-3 rounded-lg transition font-semibold">
                  Discharge Patient
            ...
            <div className="backdrop-blur-xl bg-stone-800 bg-opacity-40 border border-teal-500 border-opacity-30 rounded-xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-stone-700">
                  <div>
                    <p className="text-white font-semibold">Patient John Doe admitted</p>
                    <p className="text-xs text-stone-400">2 hours ago</p>
                  </div>
                  <span className="bg-green-500 bg-opacity-20 text-green-400 px-3 py-1 rounded-full text-xs">✓</span>
                </div>
                <div className="flex items-center justify-between pb-3 border-b border-stone-700">
                  <div>
                    <p className="text-white font-semibold">Bed 45 status updated</p>
                    <p className="text-xs text-stone-400">4 hours ago</p>
                  </div>
                  <span className="bg-teal-500 bg-opacity-20 text-teal-600 px-3 py-1 rounded-full text-xs">📝</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'beds' && (
          <div>
            <h2 className="text-2xl font-bold text-white mb-6">Bed Status</h2>
        ...
        {activeTab === 'patients' && (
          <div>
            ...
            <div className="backdrop-blur-xl bg-stone-800 bg-opacity-40 border border-teal-500 border-opacity-30 rounded-xl p-6">
              ...
              <h3 className="text-xl font-bold text-white mb-4">Admit New Patient</h3>
              ...
              <h3 className="text-xl font-bold text-white mt-8 mb-4">Active Patients</h3>
        ...
        {activeTab === 'tasks' && (
          <div>
            ...
            <div className="backdrop-blur-xl bg-stone-800 bg-opacity-40 border border-teal-500 border-opacity-30 rounded-xl p-6">
              <p className="text-stone-300">Tasks management interface coming soon...</p>
            </div>
          </div>
        )}
      </div>
